...
### M2 — render CLI
- [ ] `tedit render <template> <data> -o out.png` 全鏈路通(此階段 data 可為空物件,變數不灌)
- [ ] 退出碼 0/1/2/3/5 行為齊(4 等 M3);stdout/stderr 紀律過 e2eCli 測試
- [ ] `--scale` 經 deviceScaleFactor 生效
- **DoD:`OUT=$(tedit render a.template.json empty.yaml)` 在腳本中可靠運作,錯誤情境逐碼驗證**

### M3 — 變數綁定
- [ ] schema 補 bindings 型別(S03 定稿)+ 驗證
- [ ] resolver:注入 / 掃描 / fallback+warning / strict(exit 4)
- [ ] `tedit vars`(表格 + --json)
- [ ] 同名變數綁多處的測試樣本
- **DoD:同一模板換三份資料產出三張圖、版面不變;缺變數兩種模式行為符合 SPEC-CLI §1.2**
...
### core/resolver
- [ ] [等S03] 注入純函式 (Scene, DataMap, strict) → ResolvedScene + warnings[]
- [ ] 變數掃描函式(vars 共用)
- [ ] ❓Q2 v1 只支援本地路徑,URL 列 M6

### cli
- [x] 指令骨架+argv 解析+退出碼表 → src/cli/index.ts(2/3/5 已驗)
- [x] headless:Playwright/鎖 deviceScaleFactor/等信號/截圖 → src/cli/headless/render-png.ts(--scale 2 已驗)
- [x] render 組裝(resolver 注入除外,等 M3)→ src/cli/render.ts;CLI 出圖 vs 編輯器截圖 diff=0
...
### test-harness(橫切)
- [ ] pixelmatch 雙路徑 diff(spike 的腳本直接升級)
- [ ] 往返測試(每元素類型一樣本)
- [ ] e2eCli 情境測試(沿 chainq harness 風格)
