...
### M4 — 編輯器 v1 ✅(2026-06-15,深色 Figma 風 D20;拆 stage 1/2/3)
- [x] 畫布:engine 實例 + 選取/拖拉/控制柄縮放/旋轉/刪除/複製
- [x] 元素:文字(雙擊行內編輯)/ 圖片(上傳落地 assets/images)/ 形狀
- [x] 面板:屬性面板(可編輯)、圖層列表(z-order 拖排)
- [x] 變數綁定 UI(S04:面板開關 + 變數名)+ 畫布角標 {var}
- [x] 存檔:saveScene → PUT server 寫 templates/ + .tedit/history 時間戳副本
...
### web/editor ✅(M4)
- [x] 畫布容器+選取/控制柄接線 → src/web/ui/editor.ts(boot edit + fabric 內建)
- [x] 屬性面板(可編輯)/ 圖層列表(拖排)/ 工具列(增刪複製)
- [x] 綁定 UI+角標(S04)→ 面板開關 + {var} 角標
- [x] 存檔流(PUT server 寫入 + D10 history 副本)
- [x] 文字雙擊行內編輯接線(fabric IText;❓Q5 = IText 內建,免 overlay)
